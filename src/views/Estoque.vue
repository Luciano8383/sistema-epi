<template>
  <div class="layout-container">
    ...
    <div class="card-table">
      <div class="card-header flex-between">
        ...
        <span class="badge badge-blue">{{ cadastro_epi.length }} itens catalogados</span>
      </div>
  ...
  <td class="cargo-text">{{ item.dt_validade || '—' }}</td>
  <td class="text-bold">{{ item.quantidade }}</td>
  <!-- ... resto do código -->
</tr>
            <tr v-if="cadastro_epi.length === 0">
...
async function carregar() {
  ...
  // CORREÇÃO: Removida a vírgula e parêntese extra
  const { data, error } = await supabase
    .from('cadastro_epi')
    .select('id_epi, nome, quantidade, dt_validade, numero_ca') // Nome correto das colunas
    .order('id_epi', { ascending: false }) // Use id_epi se updated_at não existir
...
async function atualizar() {
  ...
  const { error } = await supabase
    ...
    .update({ 
      ...
      // Remova updated_at se a coluna não existir fisicamente no banco
    })
    .eq('id_epi', form.value.id_epi) // Use o nome correto da PK: id_epi

<template>
        ...
        <form @submit.prevent="salvar" class="main-form">
          <div class="form-row">
            ...
            <div class="form-group">
              ...
              <input v-model="form.cpf" type="text" id="cpf" placeholder="Ex: 5542" required>
            </div>
          </div>
          ...
          <div class="form-row">
            <div class="form-group">
              ...
              <input v-model="form.email" type="email" id="e-mail" placeholder="Ex: joao@example.com" required>
            </div>
            <div class="form-group">
  ...
  <tbody>
    <tr v-for="f in funcionario" :key="f.id">
      <!-- Coluna 1: Nome -->
...
const salvar = async () => {
  if (editandoId.value) {
    ...
    await supabase.from('funcionario').update(form).eq('id', editandoId.value);
  } else {
...
const excluir = async (id) => {
  if (confirm('Deseja realmente remover este registro?')) {
    await supabase.from('funcionario').delete().eq('id', id);
    carregar();
  }
};
...
.styled-table th {
  ...
  border-bottom: 2px solid #e2e8f0; /* Linha de destaque no topo */
}
...
.styled-table td {
  ...
  font-size: 0.95rem;
  vertical-align: middle; /* Mantém tudo alinhado ao centro verticalmente */
}

.text-bold { font-weight: 600; color: #1e293b; }

.badge {
  display: inline-block; /* Garante que o badge não quebre */
  background: #dcfce7;
  color: #166534;
  padding: 4px 10px;
  border-radius: 12px;
  font-size: 0.75rem;
  font-weight: 700;
  white-space: nowrap; /* Evita que o email quebre linha */
}
...
/* Classe utilitária de alinhamento */
